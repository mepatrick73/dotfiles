...
# transform 1 (90°) and 3 (270°) swap width/height
if args.transform in (1, 3):
    W, H = args.height, args.width
else:
    W, H = args.width, args.height

DPI = 150   # higher DPI = crisper lines
OUTPUT = args.output
PORTRAIT = H > W
BG = "#08080a"
...
# In portrait the grid sits in the bottom third; landscape bottom quarter
horizon_y = 0.18 if PORTRAIT else 0.28
vp = (0.5, horizon_y)
grid_color = "#161a1e"
grid_alpha = 0.9
lw = 0.5
...
def draw_klein(ax):
    ...
    Z = 4 * r * np.sin(V)
    ax.plot_wireframe(X / 22, Y / 22, Z / 22,
                      color="#c8d0d8", alpha=0.34, linewidth=0.42, rstride=1, cstride=1)
    ax.set_box_aspect([1, 1, 1])
    add_label(ax, "Klein", "χ = 0")


# ── Layout ────────────────────────────────────────────────────────────────────
if PORTRAIT:
    # Stack vertically — shapes centered, evenly spaced top to bottom
    # Leave bottom ~20% for the perspective grid
    sz = 0.60   # subplot width in figure fraction
    sh = 0.22   # subplot height in figure fraction
    cx = 0.5 - sz / 2  # horizontally centered

    ax1 = make_ax([cx, 0.68, sz, sh], elev=28, azim=35)
    draw_sphere(ax1)

    ax2 = make_ax([cx, 0.44, sz, sh], elev=30, azim=50)
    draw_torus(ax2)

    ax3 = make_ax([cx, 0.22, sz, sh], elev=20, azim=60)
    draw_klein(ax3)

else:
    # Landscape — diagonal stagger, size/brightness progression
    ax1 = make_ax([0.04, 0.18, 0.26, 0.54], elev=28, azim=35)
    draw_sphere(ax1)

    ax2 = make_ax([0.355, 0.21, 0.29, 0.58], elev=30, azim=50)
    draw_torus(ax2)

    ax3 = make_ax([0.675, 0.24, 0.30, 0.58], elev=20, azim=60)
    draw_klein(ax3)

fig.savefig(OUTPUT, dpi=DPI, facecolor=BG, pil_kwargs={"quality": 98})
print(f"Saved {W}x{H} ({'portrait' if PORTRAIT else 'landscape'}) → {OUTPUT}")
